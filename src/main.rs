...
impl eframe::App for CalcApp{
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx,|ui|{
            ...
            .show_ui(ui, |ui|{
                ui.selectable_value(&mut self.shoot_type, ShootType::Hourly { hours: 0.0, image_prep: false, assistant_hours: 0.0, photographer: Photographer::Ken}, "Hourly");
                ui.selectable_value(&mut self.shoot_type, ShootType::HalfDayBased { halves: 0, image_prep: false, assistant_hours: 0.0, photographer: Photographer::Ken}, "Half Day");
                //ui.selectable_value(&mut self.shoot_type, ShootType::Headshot { hours: 0.0, people: 0, editing: false, retouch: RetouchLevel::Volume, assistant_hours: 0.0, photographer: Photographer::Ken}, "Headshot");
                ui.selectable_value(&mut self.shoot_type, ShootType::NewHeadshot { heads: 0, headshot_type: HeadshotType::Large, editing: false }, "Headshot");
            });
            ...
            match &mut self.shoot_type{
                ...
                ShootType::Headshot { hours, people, editing, retouch, assistant_hours, photographer } => {
                    ui_headshot(ui, hours, people, editing, retouch, assistant_hours, photographer);
                }
            ...
            if ui.add(Label::new(format!("${}",self.calc_price().to_string())).sense(Sense::click())).on_hover_text("click to copy").clicked(){
...
fn ui_hourly(ui: &mut Ui, hours: &mut f32, image_prep: &mut bool, assistant_hours: &mut f32, photographer: &mut Photographer){
    egui::ComboBox::from_label("Photographer")
...
fn ui_half_day_based(ui: &mut Ui, halves: &mut u32, image_prep: &mut bool, assistant_hours: &mut f32, photographer: &mut Photographer){
    egui::ComboBox::from_label("Photographer")
    .selected_text(&photographer.to_string())
    .show_ui(ui, |ui|{
        ui.selectable_value(photographer, Photographer::Ken, "Ken");
...
fn ui_headshot(ui: &mut Ui, hours: &mut f32, people: &mut u32, editing: &mut bool, retouch: &mut RetouchLevel, assistant_hours: &mut f32, photographer: &mut Photographer){
    egui::ComboBox::from_label("Photographer")
    .selected_text(&photographer.to_string())
    .show_ui(ui, |ui|{
        ui.selectable_value(photographer, Photographer::Ken, "Ken");
...
fn ui_new_headshot(ui: &mut Ui, heads: &mut u32, headshot_type: &mut HeadshotType, editing: &mut bool){
...
12/people per hour
for up to {} photo hours (plus one extra hour on site for set-up & teardown)",calc_hours(*heads)-1.0);
            if ui.add(Label::new(text.clone()).sense(Sense::click())).on_hover_text("click to copy").clicked(){
